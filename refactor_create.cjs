...
const roles = [
    ...
    { role: 'receiving', path: 'receiving/documents' },
    { role: 'cart', path: 'cart/documents' },
    { role: 'cart', path: 'cart/escalations' }
];

roles.forEach(({ role, path: urlPath }) => {
    const dir = path.join('c:/Users/Huawei/TrackNgo/resources/js/pages', urlPath);
    const docFile = path.join(dir, 'Documents.tsx');
    if (!fs.existsSync(docFile)) return;

    let content = fs.readFileSync(docFile, 'utf8');

    const createContent = `import { Head, Link } from '@inertiajs/react';
...
export default function CreateDocument() {
    return (
        <TrackngoLayout
            breadcrumbs={[
                { title: 'Home', href: '/${role}' },
                { title: 'My Documents', href: '/${urlPath}' },
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <div className="rounded-2xl border border-[var(--tng-slate-200)] bg-white shadow-sm">
                    ...
                    <div className="flex items-center justify-end gap-3 border-t border-[var(--tng-slate-100)] bg-[var(--tng-slate-50)] px-8 py-5 rounded-b-2xl">
                        ...
                        <Link href="/${urlPath}" className="flex items-center gap-2 rounded-lg bg-[var(--tng-blue-600)] px-6 py-2.5 text-sm font-medium text-white shadow-md shadow-blue-600/25 transition-all hover:bg-[var(--tng-blue-700)] hover:shadow-lg">
...
`;
    ...
    fs.writeFileSync(path.join(dir, 'Create.tsx'), createContent);

    // Now update Documents.tsx
    // 1. Remove useState for showCreateModal
    content = content.replace(/const \[showCreateModal, setShowCreateModal\] = useState\(false\);\r?\n/, '');
    
    // 2. Change the button to a Link
    const oldButton = /<button\s+onClick=\{[^}]+\}\s+className="flex items-center gap-2 rounded-lg bg-\[var\(--tng-blue-600\)\][\s\S]*?<\/button>/m;
    const newLink = `<Link
                            href="/${urlPath}/create"
    ...
    content = content.replace(oldButton, newLink);

    // 3. Remove CreateDocumentModal markup block
    const modalMarkup = /\{\/\* Create Document Modal \*\/\}\r?\n\s*\{showCreateModal && \([\s\S]*?\)\}/m;
    content = content.replace(modalMarkup, '');

    // 4. Remove CreateDocumentModal function definition completely
    const functionDef = /\/\/ ── Submit New Document Modal \(Figure 10\) ───────────────────────────[\s\S]*?\}\r?\n\r?\n\/\/ Dummy import/m;
    content = content.replace(functionDef, '// Dummy import');
