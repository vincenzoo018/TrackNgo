...
export default function CreateDocumentModal({ isOpen, onClose, departments = [], documentTypes = [], users = [] }: CreateDocumentModalProps) {
    ...
    const { data, setData, post, processing, errors, reset } = useForm({
        title: '',
        type_id: '',
        department_id: receivingDept ? receivingDept.department_id : '',
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setIsUrgent(false);
            reset();
            setData('department_id', receivingDept ? receivingDept.department_id : '');
        }
    }, [isOpen, receivingDept]);
                                ...
                                <div className="space-y-5 pl-7">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            {errors.department_id && <p className="text-[11px] text-red-600 mt-1 font-medium">{errors.department_id}</p>}
                                        </div>
                                        <div>
                                            <label className="mb-1.5 block text-xs font-medium text-[var(--tng-slate-700)]">
                                                Sender (Registering Officer)
                                            </label>
                                    ...
                                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                                        <label className="flex items-start gap-3 cursor-pointer">
                                            ...
                                            <div>
                                                ...
                                                <span className="block text-[10px] text-amber-700/80 mt-0.5 leading-tight">
                                        ...
                                        <div className={`mt-3 transition-all duration-300 ${isUrgent ? 'opacity-100 max-h-20' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                                            <input
                                                ...
                                                className="h-9 w-full rounded-lg border border-amber-200 bg-white px-3 text-sm text-[var(--tng-slate-900)] placeholder:text-amber-400/70 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                                                required={isUrgent}
                                            />
                                        </div>
                                    </div>
