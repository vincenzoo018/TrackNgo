...
export default function ReportsIndex({
    ...
    const handleExportCsv = () => {
        try {
            ...
            csvContent += `Access Scope,"${isFullAccess ? 'System-Wide Oversight' : 'Departmental Analytics'}"\n`;
    ...
    return (
        <TrackngoLayout>
            ...
            <div className="space-y-5 pb-12">
                ...
                {/* ── KPI METRICS CARDS (Standardized System Blue) ───────────── */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5 shrink-0">
                    <StatCard
                        title="Total Documents"
                        value={metrics.totalProcessed}
                        sublabel={`${metrics.completedCount} Completed • ${metrics.activeCount} Ongoing`}
                        icon={BarChart3}
                    />

                    <StatCard
                        title="ARTA SLA Compliance"
                        value={`${metrics.slaComplianceRate}%`}
                        sublabel={`${metrics.slaCompliant} Compliant Docs (RA 11032)`}
                        icon={CheckCircle2}
                    />
                    ...
                    <StatCard
                        ...
                        sublabel={metrics.nearDeadline > 0 ? `${metrics.nearDeadline} near deadline (≤2d)` : 'No impending breaches'}
                        icon={AlertTriangle}
                    />

                    <StatCard
                        title="Avg Turnaround Time"
                        value={`${metrics.avgTurnaroundDays} Days`}
                        sublabel="Processing benchmark (3–7d target)"
                        icon={Clock}
                    />
                </div>
                                            ...
                                            <tr key={log.id} className="group transition-colors odd:bg-white even:bg-slate-50/75 hover:bg-blue-50/40">
                                                ...
                                                <td className="py-2.5 px-4 text-right whitespace-nowrap">
                                                    <TableActionButtons
                                                        ...
                                                        onDelete={() => {
                                                            if (confirm(`Delete log record #${log.id}?`)) {
                ...
                <div className="hidden">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs mb-6 border border-slate-200 p-3 rounded bg-slate-50/50">
                            ...
                            <div>
                                <p><span className="font-semibold text-slate-600">Access Scope:</span> {isFullAccess ? 'System-Wide Oversight (All Users & Departments)' : 'Departmental Analytics'}</p>
