...
export default function AdminDocumentsIndex() {
    ...
    // Active documents or archived depending on tab
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedIds, setSelectedIds] = useState<number[]>([]);
    const [activeTab, setActiveTab] = useState<'all' | 'received' | 'ongoing' | 'sent' | 'returned' | 'archived'>('all');
    ...
    const tabCounts = useMemo(() => {
        const counts = { all: rawDocuments.length, received: 0, ongoing: 0, sent: 0, returned: 0, archived: 0 };
        rawDocuments.forEach((doc: any) => {
            if (isFinalizedOrArchived(doc.status)) {
                counts.archived++;
            }
            const std = getStandardizedStatus(doc.status).toLowerCase() as keyof typeof counts;
            if (counts[std] !== undefined && std !== 'archived') {
                counts[std]++;
            }
        });
        return counts;
    ...
    const filteredDocs = useMemo(() => {
        let result = documents.filter((doc: any) => {
            ...
            // Tab filtering logic
            if (activeTab === 'archived') {
                if (!isFinalizedOrArchived(doc.status)) return false;
            } else if (activeTab !== 'all' && stdStatus.toLowerCase() !== activeTab) {
                return false;
            }
            ...
            <div className="space-y-5">
                ...
                <TabNavigation
                    tabs={[
                        ...
                        { id: 'received', label: 'Received', icon: <Inbox className="h-4 w-4" />, count: tabCounts.received },
                        { id: 'ongoing', label: 'Ongoing', icon: <Clock className="h-4 w-4" />, count: tabCounts.ongoing },
                        { id: 'sent', label: 'Sent', icon: <Send className="h-4 w-4" />, count: tabCounts.sent },
                        { id: 'returned', label: 'Returned', icon: <RotateCcw className="h-4 w-4" />, count: tabCounts.returned },
                        { id: 'archived', label: 'Archived', icon: <Archive className="h-4 w-4" />, count: tabCounts.archived },
                    ]}
                    activeTab={activeTab}
                    onChange={(tab) => {
                        setActiveTab(tab);
                                ...
                                {paginatedDocs.map((doc: any, idx: number) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <button className="rounded-lg p-1.5 text-[var(--tng-slate-400)] transition-all hover:bg-slate-100 hover:text-[#0066cc] focus:outline-none focus:ring-2 focus:ring-blue-400/50" title="View QR Code" aria-label="View QR">
